...
export function NavMesh(props) {
  ...
  useMemo(async () => {
    ...
    //loader.load('/level3_navmesh.glb', { mergeConvexRegions: false } )
    await loader
      .load("navmesh_applied.glb", { mergeConvexRegions: false })
      .then((navMesh) => {
        setNavMesh(navMesh);
      });
  }, []);
  ...
  const { nodes, materials } = useGLTF("/navmesh_applied.glb");
  return (
    <>
      <mesh
        ref={mesh}
